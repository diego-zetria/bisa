...
module.exports = function makeSession(deps) {
  const {
    ...
    getExtraSystemPrompt,      // opcional: string por turno → --append-system-prompt adicional (ex.: Modo Estudo do caderno)
    CLAUDE_CMD,
    USER_SHELL,
    broadcast,
  ...
  const buildArgs = (sessionId) => {
    const args = [
      ...
      // texto token a token (stream_event): sem isso o texto só chega por
      // mensagem COMPLETA e a resposta longa "cai de uma vez" — vídeos
      // 2026-07-19 mostraram 16-32s de tela parada pós-ferramentas, e foi
      // exatamente nesse vácuo que o usuário abandonou para outro app.
    ...
    if (getExtraSystemPrompt) {
      const extra = getExtraSystemPrompt();
      if (extra) args.push('--append-system-prompt', String(extra));
    }
    if (sessionId) {
  ...
  const runTurn = (text, attachments) => new Promise((resolve, reject) => {
    ...
    // --include-partial-messages: texto/thinking chegam token a token via
    // stream_event ANTES da mensagem assistant completa. Depois do primeiro
    // delta, os blocos text/thinking das mensagens completas são pulados
    // (já foram transmitidos) — CLI antigo sem stream_event cai no caminho velho.
        ...
        case 'stream_event': {
          ...
          if (e.type === 'content_block_start' && e.content_block && e.content_block.type === 'text') {
            ...
            if (e.delta.type === 'text_delta' && e.delta.text) {
              sawPartials = true;
              broadcast({ type: 'llm.text', delta: e.delta.text });
            } else if (e.delta.type === 'thinking_delta' && e.delta.thinking) {
              sawPartials = true;
              broadcast({ type: 'llm.thinking', delta: String(e.delta.thinking).slice(0, 4000) });
            }
          }
          break;
        }
        ...
        case 'assistant': {
          ...
          for (const block of content) {
            if (block.type === 'text' && block.text) {
              // com partials o texto já foi transmitido delta a delta — só marca
              // a fronteira p/ o sentinela pós-ferramenta continuar correto.
              if (sawPartials) { sentTextThisTurn = true; }
              else {
              ...
              // HUD: pensamento visível (card colapsável no cliente); com
              // partials os deltas já foram transmitidos via stream_event.
              if (block.thinking && !sawPartials) broadcast({ type: 'llm.thinking', delta: String(block.thinking).slice(0, 4000) });
            }
          }
          break;
        }
