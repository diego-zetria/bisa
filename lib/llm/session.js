...
const TOOL_SUMMARY_PT = {
  ...
  WebSearch:(input) => `Pesquisando na internet`,
...
module.exports = function makeSession(deps) {
  ...
  // Session state file: persists session_id for --resume
  const SESSION_FILE = path.join(CWD, '.meta', 'llm-session.json');
  ...
  const writeSessionId = (id) => {
    try {
      fs.mkdirSync(path.dirname(SESSION_FILE), { recursive: true });
      fs.writeFileSync(SESSION_FILE, JSON.stringify({ sessionId: id }, null, 2) + '\n', 'utf8');
        ...
        case 'assistant': {
          ...
          for (const block of content) {
            if (block.type === 'text' && block.text) {
              broadcast({ type: 'llm.text', delta: block.text });
              ...
              // Thinking blocks: don't broadcast, just skip
            }
          }
          break;
        }
        ...
        case 'user': {
          ...
          for (const block of content) {
            if (block.type === 'tool_result') {
              // We don't know the tool name at this point — just signal 'done'
              // without a name (the UI can match by order if needed)
              broadcast({ type: 'llm.tool', name: null, summaryPt: null, status: 'done' });
            }
          }
          break;
        }
