...
module.exports = function makeLlm(deps) {
  ...
  const microTask = (kind, text, opts = {}) => {
    const route = policy.resolveMicroRoute();
    if (route === 'off') return Promise.resolve(null);
    const viaHeadless = () =>
      headless.runClaudeHeadless(text, CWD, opts.timeoutMs || 30000, { feature: 'micro-' + kind })
        .then((out) => out.trim() || null)
        .catch(() => null);
    if (route === 'claude-p') return viaHeadless();
    // route === 'api' — API falhando (chave inválida/rede/budget) não pode
    // apagar a feature: cai para o claude -p, como se a chave não existisse.
    return api.microTask(kind, text, opts).then((out) => out || viaHeadless());
  };
