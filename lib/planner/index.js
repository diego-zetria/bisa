...
module.exports = function makePlanner({ requireAuth, CWD, dispatchNotification, quickAddLlm }) {
  ...
  // ---- ICS pela UI ("Conectar calendário" no hub) ---------------------------
  // env BISA_ICS_URL continua vencendo; sem env, vale a URL gravada aqui.
  const ICS_CFG = path.join(CWD, '.meta', 'planner-ics.json');
  const readIcsUrl = () => {
    try { return String(JSON.parse(fs.readFileSync(ICS_CFG, 'utf8')).url || '').trim(); }
    catch { return ''; }
  };
  ics.setUrlResolver(readIcsUrl);

  router.get('/planner/ics', requireAuth, (_req, res) => {
    res.json({ url: readIcsUrl(), env: !!process.env.BISA_ICS_URL });
  });
  // POST {url} — valida BUSCANDO o calendário antes de gravar (URL errada não
  // fica salva); url vazia desconecta. webcal:// é normalizado para https://.
  router.post('/planner/ics', requireAuth, async (req, res) => {
    let url = String((req.body || {}).url || '').trim();
    if (/^webcal:\/\//i.test(url)) url = 'https://' + url.slice(9);
    if (url && !/^https?:\/\//i.test(url)) return res.status(400).json({ error: 'URL deve começar com https:// (ou webcal://)' });
    try {
      if (url) await require('node-ical').async.fromURL(url);
    } catch (e) {
      return res.status(400).json({ error: 'não consegui ler esse calendário: ' + e.message });
    }
    try {
      fs.mkdirSync(path.dirname(ICS_CFG), { recursive: true });
      fs.writeFileSync(ICS_CFG, JSON.stringify({ url }, null, 2) + '\n', 'utf8');
      ics.clearCache();
      const r = url ? await ics.getEventsForDate(store.todayStr()) : { events: [] };
      res.json({ ok: true, connected: !!url, todayEvents: (r.events || []).length });
  ...
  router.post('/planner/task', requireAuth, async (req, res) => {
    try {
      ...
      // Dedup: a MESMA tarefa aberta no mesmo dia (texto normalizado) não entra
      // duas vezes — achado da análise automática de vídeo de 2026-07-13
      // ("15:00 dentista" duplicada no Hoje). Devolve a existente.
      const norm = (s) => String(s || '').toLowerCase().replace(/\s+/g, ' ').trim();
      const dupId = Object.keys(state.tasks).find((tid) => {
        const t = state.tasks[tid];
        return t && !t.done && t.date === finalDate && norm(t.text) === norm(parsed.text);
      });
      if (dupId) {
        return res.json({ ok: true, duplicate: true, task: store.hydrateTask(state, dupId) });
      }
