...
// URL resolvida a cada chamada: env vence; senão a config gravada pela UI
// ("Conectar calendário" no hub — ver /planner/ics no index). O resolver é
// injetado pelo factory do planner porque só ele conhece o CWD.
let resolveExtraUrl = () => '';
function setUrlResolver(fn) { resolveExtraUrl = fn; }
const currentUrl = () => process.env.BISA_ICS_URL || resolveExtraUrl() || '';

// { url, data: raw ical events map, fetchedAt: timestamp }
let cache = null;
...
async function fetchRaw(url, fetcher) {
  ...
  const now = Date.now();
  if (cache && cache.url === url && (now - cache.fetchedAt) < CACHE_TTL) return cache.data;
...
async function getEventsForDate(dateStr, fetcher) {
  ...
  const data = await fetchRaw(url, fetcher);
  const events = [];
...
module.exports = { getEventsForDate, clearCache, setUrlResolver };
