...
module.exports = function makePush(deps) {
  ...
  return {
    router,
    // Liga dispatchNotification → push: toda notificação não-silenciosa vira
    // notificação nativa nos dispositivos inscritos. Recebe o onNotification
    // do lib/notify (registro de listeners).
    bridgeNotifications: (onNotification) => {
      if (typeof onNotification !== 'function') return;
      onNotification((n) => {
        if (n.silent) return;
        sendAll({ title: 'bisa', body: n.text, tag: 'notify', url: '/' })
          .catch((e) => console.warn('[bisa/push] bridge send failed:', e.message));
      });
    },
