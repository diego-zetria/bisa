...
module.exports = function makeNotify(deps) {
  ...
  // Extra sinks beyond the WS broadcast (e.g. Web Push bridge in lib/push).
  const listeners = [];
  const onNotification = (fn) => listeners.push(fn);
  ...
  const dispatchNotification = (notif) => {
    ...
    for (const fn of listeners) {
      try { fn(notif); } catch (e) { console.warn('[bisa] notification listener failed:', e.message); }
    }
  };
  ...
  return { router, dispatchNotification, logNotificationToCodex, onNotification };
};
