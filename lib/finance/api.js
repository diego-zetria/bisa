...
module.exports = function makeFinanceRouter(deps) {
  ...
  // GET /finance/dash?months=N — série multi-mês para os dashboards (2026-07-19).
  // Agregados leves por mês (renda/gasto/saldo/categoria/envelope) do ledger
  // manual + totais do Actual quando configurado (best-effort, paralelo).
  // Detalhe por transação fica fora — o drill busca /finance/summary?month=.
  router.get('/finance/dash', requireAuth, async (req, res) => {
    const n = Math.max(1, Math.min(24, parseInt(req.query.months, 10) || 6));
    const list = [];
    let [y, m] = thisMonth().split('-').map(Number);
    for (let i = 0; i < n; i++) { list.unshift(`${y}-${String(m).padStart(2, '0')}`); m--; if (!m) { m = 12; y--; } }
    try {
      const all = loadTransactions();
      const months = list.map((month) => {
        let income = 0, expense = 0;
        const byCategory = {}, byBucket = {};
        for (const t of all) {
          if (t.date.slice(0, 7) !== month || t.pending) continue;
          if (t.kind === 'income') income += t.amount;
          else {
            expense += t.amount;
            byCategory[t.category] = r2((byCategory[t.category] || 0) + t.amount);
            if (t.bucket) byBucket[t.bucket] = r2((byBucket[t.bucket] || 0) + t.amount);
          }
        }
        return { month, income: r2(income), expense: r2(expense), net: r2(income - expense), byCategory, byBucket };
      });
      if (actual.configured()) {
        await Promise.all(months.map(async (row) => {
          try {
            const a = await actual.monthSummary(row.month);
            row.income = r2(row.income + a.income);
            row.expense = r2(row.expense + a.expense);
            row.net = r2(row.income - row.expense);
            for (const [k, v] of Object.entries(a.byCategory || {})) row.byCategory[k] = r2((row.byCategory[k] || 0) + v);
          } catch { /* Actual fora do ar não derruba o dash */ }
        }));
      }
      const profile = profileMod.loadProfile();
      res.json({
        months,
        allocation: (profile && profile.allocation) || null,
        allocationFixed: (profile && profile.allocationFixed) || null,
        allocationRest: (profile && profile.allocationRest) || null,
      });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
