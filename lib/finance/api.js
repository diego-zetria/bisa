...
module.exports = function makeFinanceRouter(deps) {
  ...
  // PATCH /finance/budget {category, amount} — edit a planned budget line's value.
  router.patch('/finance/budget', requireAuth, (req, res) => {
    ...
    const line = profileMod.updateBudgetAmount(String(category), n);
    if (!line) return res.status(404).json({ error: 'budget line not found' });
    res.json({ ok: true, line });
  });

  // PUT /finance/budget/order {order:[category,...]} — reordena as linhas do orçamento.
  router.put('/finance/budget/order', requireAuth, (req, res) => {
    const order = (req.body || {}).order;
    if (!Array.isArray(order)) return res.status(400).json({ error: 'order array required' });
    const budget = profileMod.reorderBudget(order.map(String));
    if (!budget) return res.status(404).json({ error: 'no profile/budget' });
    res.json({ ok: true });
  });
