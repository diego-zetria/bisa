...
function makeIndexer({ CWD, broadcast }) {
  ...
  // --------------------------------------------------------------------------
  // Search: entities + journal full-text, accent-insensitive, ranked.
  // Deterministic score (no embeddings): exact name/slug 100, name/slug
  // prefix 80, contiguous substring 60, all words present anywhere 40 —
  // so "ana aniversário" finds Ana even when the words aren't adjacent.
  // --------------------------------------------------------------------------
  function scoreText(norm, words, { name = '', slug = '', haystack }) {
    if (name === norm || slug === norm) return 100;
    if ((name && name.startsWith(norm)) || (slug && slug.startsWith(norm))) return 80;
    if (haystack.includes(norm)) return 60;
  ...
  function search(q) {
    ...
    for (const [, e] of state.entities) {
      ...
      const score = scoreText(norm, words, {
        name: normalizeForSearch(e.fm.name || ''),
        slug: normalizeForSearch(e.slug),
        haystack,
      });
      if (score > 0) {
        entities.push({
    ...
    for (const block of blocks) {
      ...
      const score = scoreText(norm, words, { haystack: normalizeForSearch(body) });
      if (score > 0) {
        journal.push({ date, excerpt: makeExcerpt(body), score });
      }
    }
    journal.sort((a, b) => b.score - a.score || b.date.localeCompare(a.date));
